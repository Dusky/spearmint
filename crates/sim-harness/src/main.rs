...
fn report(field: &dyn CellField, table: &ElementTable, options: Options) {
    if options.census {
        for element in table.iter() {
            let count = count_of(field, element.id);
            eprintln!("{:>8}: {count}", element.name);
        }
    }
    if options.dump {
        dump(field, table);
...
fn count_of(field: &dyn CellField, id: sim_core::ElementId) -> usize {
...
fn dump(field: &dyn CellField, table: &ElementTable) {
    let Some(bounds) = field.bounds() else {
        return;
    };
    for y in bounds.min_y..=bounds.max_y {
